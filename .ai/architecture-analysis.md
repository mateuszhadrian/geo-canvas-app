...
nie wymaga zmiany żadnego innego pliku... *z wyjątkiem trzech miejsc opisanych w sekcji 3*.
...
export interface ShapeDefinition<S extends BaseShape = BaseShape> {
  ...
  applyHandleDrag: (                                  // delta → ShapeUpdate
    start: unknown,
    kind: string,
    ldx: number,
    ldy: number,
...
type ShapeGeometry<S extends BaseShape> = Partial<Omit<S, 'id' | 'stroke' | 'strokeWidth' | 'opacity'>>
...
- Zawsze będą dwa rodzaje: *elementy pierwotne* i *elementy relacyjne (connectors)*
