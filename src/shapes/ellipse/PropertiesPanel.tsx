...
}: { label: string; value: number; min?: number; onCommit: (v: number) => void }) {
  const [prevProp, setPrevProp] = useState(value)
  const [local, setLocal] = useState(String(Math.round(value)))
  if (prevProp !== value) { setPrevProp(value); setLocal(String(Math.round(value))) }
...
export function EllipsePropertiesPanel({ shape }: { shape: EllipseShape }) {
  ...
  return (
    <div className="flex flex-col gap-2 pt-1">
      <NumericField label="Radius X" value={shape.radiusX} min={5}
        onCommit={(v) => {
          updateShapeTransient(shape.id, { radiusX: v })
          commitShapeUpdate(shape.id, { radiusX: shape.radiusX }, { radiusX: v })
        }}
      />
      <NumericField label="Radius Y" value={shape.radiusY} min={5}
        onCommit={(v) => {
